...
## Highlights of version 3.10.0
- **Major Hardware Refactoring**: Integrated support for a 240x240 ST7789 TFT display and a GPS module.
- **New Pin Mapping**: The default pin configuration in `include/config.h` has been significantly updated to accommodate the new peripherals. **This is a breaking change.**
- **New Libraries**: Uses Adafruit GFX and Adafruit ST7789/ST7735 libraries for TFT support. Added TinyGPS++ for GPS data parsing.
- **Enhanced UI**: The web interface now includes dedicated cards for the TFT display, GPS status, and other sensors, providing real-time data.
- **New Tasks**: Added FreeRTOS tasks for handling TFT display updates and GPS data processing in the background.
...
- **Libraries:** Adafruit BusIO, Adafruit GFX, Adafruit ST7735 and ST7789 Library, Adafruit NeoPixel, DHT sensor library, TinyGPS++ (auto-installed via platformio.ini).
