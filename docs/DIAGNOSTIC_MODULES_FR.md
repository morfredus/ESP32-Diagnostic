# ESP32 Diagnostic Suite – Modules de diagnostic (v3.21.0)
...
2. **Connectivité** – lance des scans Wi-Fi et vérifie la diffusion mDNS.
...
Des diagnostics personnalisés peuvent être ajoutés via `registerCustomDiagnostics()` dans `le fichier source principal`. Chaque callback reçoit :
